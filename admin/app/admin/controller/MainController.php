...
class MainController extends AdminBaseController {
    ...
    public function index(){
        ...
        if($source){
            ...
            foreach($source as $k=>$v){
                ...
                $data_source['color'][]=$color[$k % $color_count];
                $data_source['nums_per'][]=$nums_totoal > 0 ? round($v['nums']*100/$nums_totoal) : 0;
            }
        }
        ...
        $color_v_n_count=count($color_v_n);
        if($login_type){

            foreach($login_type as $k=>$v){
                $data_type['v_n'][]=['value'=>$v['nums'],'name'=>$v['login_type'],'itemStyle'=>['color'=>$color_v_n[$k % $color_v_n_count]]];
        ...
        $votes_list=Db::name('user')
                    ->field("id,user_nickname,avatar,avatar_thumb,votestotal")
                    ->where("user_type=2")
        ...
        $rich_list=Db::name('user')
                    ->field("id,user_nickname,avatar,avatar_thumb,consumption")
                    ->where("user_type=2")
        ...
        switch($basic_type){
            case '1':
                if($periodType=='hourly'){
                    ...
                    foreach($rs['name'] as $k=>$v){
                        ...
                        $newUsers_a= $newusers_android[0]['hourValue'][$k] ?? 0;
                        $newUsers_i= $newusers_ios[0]['hourValue'][$k] ?? 0;
                    ...
                    /* value */
                    foreach($rs['name'] as $k=>$v){
                        $newUsers_a= $newusers_android[$k]['value'] ?? 0;
                        $newUsers_i= $newusers_ios[$k]['value'] ?? 0;
            ...
            case '2':
                if($periodType=='hourly'){
                    /* hourValue */
                    foreach($rs['name'] as $k=>$v){
                        $launches_a= $launches_android[0]['hourValue'][$k] ?? 0;
                        $launches_i= $launches_ios[0]['hourValue'][$k] ?? 0;
                    ...
                    foreach($rs['name'] as $k=>$v){
                        ...
                        $launches_a= $launches_android[$k]['value'] ?? 0;
                        $launches_i= $launches_ios[$k]['value'] ?? 0;
            ...
            case '3':
                    ...
                    foreach($rs['name'] as $k=>$v){
                        $durations_a= $durations_android[$k] ?? 0;
                        $durations_i= $durations_ios[$k] ?? 0;
            ...
            case '4':
                if($periodType=='hourly'){
                    ...
                    foreach($rs['name'] as $k=>$v){
                        ...
                        $activityUsers_a= $activeusers_android[0]['hourValue'][$k] ?? 0;
                        $activityUsers_i= $activeusers_ios[0]['hourValue'][$k] ?? 0;
                    ...
                    foreach($rs['name'] as $k=>$v){
                        ...
                        $activityUsers_a= $activeusers_android[$k]['value'] ?? 0;
                        $activityUsers_i= $activeusers_ios[$k]['value'] ?? 0;
            ...
            case '5':
                if($periodType=='hourly'){
                    ...
                    foreach($rs['name'] as $k=>$v){
                        ...
                        $retentions_a= $retentions_android[0]['hourValue'][$k] ?? 0;
                        $retentions_i= $retentions_ios[0]['hourValue'][$k] ?? 0;
                    ...
                    foreach($rs['name'] as $k=>$v){
                        ...
                        $retentions_a= $retentions_android[$k]['value'] ?? 0;
                        $retentions_i= $retentions_ios[$k]['value'] ?? 0;
    ...
    protected function getCharge($start,$end){
        ...
        $charge_color=['#f44957','#5bc189','#33c5f1','#f8c299','#26a17b'];
    ...
    function export(){
        ...
        switch($action){
            case '1':
                ...
                if($periodType!='hourly'){
                    for($i=$starttime,$n=0;$i<$endtime;$i+=60*60*24,$n++){
                        $newUsers_a= $newusers_android[$n]['value'] ?? 0;
                        $newUsers_i= $newusers_ios[$n]['value'] ?? 0;
                        
                        $launches_a= $launches_android[$n]['value'] ?? 0;
                        $launches_i= $launches_ios[$n]['value'] ?? 0;
                        
                        $duration_a= $durations_android[$n] ?? 0;
                        $duration_i= $durations_ios[$n] ?? 0;
                        
                        $activityUsers_a= $activeusers_android[$n]['value'] ?? 0;
                        $activityUsers_i= $activeusers_ios[$n]['value'] ?? 0;
                    ...
                    for($n=0;$n<24;$n++){
                        ...
                        $newUsers_a= $newusers_android[0]['hourValue'][$n] ?? 0;
                        $newUsers_i= $newusers_ios[0]['hourValue'][$n] ?? 0;
                        
                        $launches_a= $launches_android[0]['hourValue'][$n] ?? 0;
                        $launches_i= $launches_ios[0]['hourValue'][$n] ?? 0;
                        
                        
                        $activityUsers_a= $activeusers_android[0]['hourValue'][$n] ?? 0;
                        $activityUsers_i= $activeusers_ios[0]['hourValue'][$n] ?? 0;
            ...
            case '4':
                ...
                $xlsCell  = array('date','total','ali','wx','apple','usdt');
