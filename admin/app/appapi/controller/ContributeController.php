...
class ContributeController extends HomebaseController {
	...
	function index(){
        ...
        /* 日榜 */
		$list_day=Db::name('user_voterecord')
		...
		foreach($list_day as $k=>$v){
			...
			$list_day[$k]['total']=$total;
		}
		$this->assign("list_day",$list_day);
        
        $list_day_total=Db::name('user_voterecord')
            ->where(" action in (1,2) and uid='{$uid}' and addtime >={$today_start} and addtime < {$today_end} ")
            ->sum('total');
        if(!$list_day_total){
            $list_day_total=0;
        ...
        /* 周榜 */
        $list_week=Db::name('user_voterecord')
            ->field("fromid as uid,sum(total) as total")
		...
		foreach($list_week as $k=>$v){
			...
			$list_week[$k]['total']=$total;
		}
		$this->assign("list_week",$list_week);
        
        $list_week_total=Db::name('user_voterecord')
            ->where(" action in (1,2) and uid='{$uid}' and addtime >={$week_start} and addtime < {$week_end} ")
            ->sum('total');
        if(!$list_week_total){
            $list_week_total=0;
        ...
        /* 月榜 */
        $list_month=Db::name('user_voterecord')
            ->field("fromid as uid,sum(total) as total")
		...
		foreach($list_month as $k=>$v){
			...
			$list_month[$k]['total']=$total;
		}
		$this->assign("list_month",$list_month);
        
        $list_month_total=Db::name('user_voterecord')
                ->where(" action in (1,2) and uid='{$uid}' and addtime >={$month_start} and addtime < {$month_end} ")
                ->sum('total');
        if(!$list_month_total){
            $list_month_total=0;
        ...
        /* 总榜 */
        $list_all=Db::name('user_voterecord')
                ->field("fromid as uid,sum(total) as total")
		...
		foreach($list_all as $k=>$v){
			...
			$list_all[$k]['total']=$total;
		}
		$this->assign("list_all",$list_all);
        
        $list_all_total=Db::name('user_voterecord')
                ->where(" action in (1,2) and uid='{$uid}' ")
                ->sum('total');
        if(!$list_all_total){
            $list_all_total=0;
