...
class livebackController extends HomebaseController {
	...
	*/
	...
	public function callbacklog($msg){
		//file_put_contents(CMF_ROOT.'log/think/appapi/liveback/callbacklog_'.date('Y-m-d').'.txt',date('Y-m-d H:i:s').' 提交参数信息 :'.$msg."\r\n",FILE_APPEND);
	}
	...
	public function stopRoom($uid='',$stream=''){
        ...
        //file_put_contents(CMF_ROOT.'log/think/appapi/liveback/stoproom_'.date('Y-m-d').'.txt',date('Y-m-d H:i:s').' 提交参数信息 :'.$uid.'--'.$stream."\r\n",FILE_APPEND);
        $where['islive']=1;
		if($uid){
            $where['uid']=$uid;
		}else{
		...
		//file_put_contents(CMF_ROOT.'log/think/appapi/liveback/stoproom_'.date('Y-m-d').'.txt',date('Y-m-d H:i:s').' 直播记录info信息 :'.json_encode($info)."\r\n",FILE_APPEND);
		...
		if($info){
            ...
            //file_put_contents(CMF_ROOT.'log/think/appapi/liveback/stoproom_'.date('Y-m-d').'.txt',date('Y-m-d H:i:s').' live_type信息 :'.$live_type."\r\n",FILE_APPEND);
    ...
    public function uplive(){
        ...
        // file_put_contents(CMF_ROOT.'log/think/appapi/liveback/uplive_'.date('Y-m-d').'.txt',date('Y-m-d H:i:s').' 提交参数信息 :'.'OK'."\r\n",FILE_APPEND);
        echo 'OK';
    ...
    /**
     * 声网云端播放器状态改变回调
     * https://doc.shengwang.cn/api-ref/media-pull/restful/player-state-changed
     * @return [type] [description]
     */
    public function swCloudPlayerCallback(){

    	$response = file_get_contents("php://input");
    	$this->swCallbackLog('callback response:'.$response);

    	$res_arr = json_decode($response,true);
    	$eventType=$res_arr['eventType'];

    	//云端播放器创建成功
    	//https://doc.shengwang.cn/api-ref/media-pull/restful/player-created
    	/*if($eventType==1){ 
    	...
    	//https://doc.shengwang.cn/api-ref/media-pull/restful/player-state-changed
    	if($eventType==4){
    		$payload = $res_arr['payload'];
    		$player = $payload['player'];
    		$stream = $player['channelName'];
    		$status = $player['status'];

    		$update_data=[
    			'sw_player_status'=>$status
    		];

    		if($status=='running'){
    			$update_data['islive']=1;
    			$update_data['sw_player_id']=$player['id']; //云端播放器id
    		}


    		//更改直播声网云端播放器状态
    		Db::name("live")->where(['stream'=>$stream])->update($update_data);
    	}

    	//云端播放器销毁成功
    	//https://doc.shengwang.cn/api-ref/media-pull/restful/player-destroyed
    	/*if($eventType==3){
    ...
    private function swCallbackLog($msg){
    	file_put_contents(CMF_ROOT.'log/think/appapi/liveback/swCallbackLog_'.date('Y-m-d').'.txt',date('Y-m-d H:i:s').' 提交参数信息 :'.$msg."\r\n\r\n",FILE_APPEND);
    }
