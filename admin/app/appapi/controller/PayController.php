...
class PayController extends HomebaseController {
	...
	*/
	...
	public function notify_ios(){
        ...
        $chargeinfo=claw_normalize_charge_rule($chargeinfo);
	...
	public function notify_usdt(){
		...
		$this->logusdt("data:".json_encode($data));

		$config=claw_get_bepusdt_config();
		if(empty($config['api_token'])){
			$this->logusdt("BEpusdt token missing");
			echo "fail";
			return;
		}
		...
		if(!$signature){
			$this->logusdt("signature missing");
			echo "fail";
			return;
		}

		$localSign=claw_bepusdt_sign($data,$config['api_token']);
		if(!hash_equals($localSign,strtolower((string)$signature))){
			$this->logusdt("signature invalid local:".$localSign." remote:".$signature);
			echo "fail";
			return;
		}

		$orderid=checkNull($data['order_id'] ?? '');
		$status=(int)($data['status'] ?? 0);
		if(!$orderid){
			$this->logusdt("order_id missing");
			echo "fail";
			return;
		}

		if($status!==2){
			$this->logusdt("ignore status:".$status." orderid:".$orderid);
			echo "success";
			return;
		}

		$order=Db::name("charge_user")->where(['orderno'=>$orderid,'type'=>7])->find();
		if(!$order){
			$this->logusdt("order not found:".$orderid);
			echo "fail";
			return;
		}

		$notifyAmount=(float)($data['amount'] ?? 0);
		if(abs((float)$order['money']-$notifyAmount)>0.01){
			$this->logusdt("amount mismatch order:".$order['money']." notify:".$notifyAmount);
			echo "fail";
			return;
		}
		...
		if($actualAmount<=0){
			$this->logusdt("actual_amount invalid:".$actualAmount);
			echo "fail";
			return;
		}

		$tradeNo=$data['block_transaction_id'] ?? '';
		if(!$tradeNo){
			$tradeNo=$data['trade_id'] ?? '';
		}

		$res=handelCharge(
			['orderno'=>$orderid,'type'=>7],
			['trade_no'=>$tradeNo]
		);
		if($res==0){
			$this->logusdt("handelCharge failed orderid:".$orderid);
			echo "fail";
			return;
		}

		$this->logusdt("success orderid:".$orderid);
		echo "success";
		return;
	}
	...
	public function logusdt($msg){
		$path=CMF_ROOT.'log/think/appapi/pay/logusdt_'.date('Y-m-d').'.txt';
		if(!is_dir(dirname($path))){
			mkdir(dirname($path),0755,true);
		}
		file_put_contents($path,date('Y-m-d H:i:s').'  msg:'.$msg."\r\n",FILE_APPEND);
	}
