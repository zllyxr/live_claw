...
class PaypalbackController extends HomebaseController {
	...
	public function notify_paypal() {
		...
		if($verified) {//验证成功
			...
			if($notify_type=="coin_charge"){//星币充值
	...
	public function logpaypal($msg){
		file_put_contents(CMF_ROOT.'log/think/appapi/paypalback/logpaypal_'.date('Y-m-d').'.txt',date('Y-m-d H:i:s').'  msg:'.$msg."\r\n",FILE_APPEND);
	}
