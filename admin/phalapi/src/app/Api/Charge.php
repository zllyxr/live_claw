...
class Charge extends Api {
	...
	public function getRules() {
		return array(
			'getAliOrder' => array(
				...
				'coin' => array('name' => 'coin', 'type' => 'string',  'require' => true, 'desc' => '星币'),
	...
	public function getWxOrder() {
		...
		$paramarr = array(
			...
			"body"        =>    \PhalApi\T("充值{coin}星币",array('coin'=>$coin)),
	...
	*/
	...
	/**
	 * USDT支付获取订单
	 * @desc 创建BEpusdt订单并返回收银台地址
	 * @return int code 操作码，0表示成功
	 * @return array info
	 * @return string info[0].orderid 商户订单号
	 * @return string info[0].trade_id BEpusdt交易ID
	 * @return string info[0].payment_url 收银台地址
	 * @return string info[0].token 收款地址
	 * @return string msg 提示信息
	 */
	public function getUsdtOrder() {
		$rs = array('code' => 0, 'msg' => '', 'info' => array());
		...
		if($coin==0 || (float)$money<=0){
			$rs['code']=1002;
			$rs['msg']=\PhalApi\T('信息错误');
			return $rs;
		}

		$bepusdt=\App\getBepusdtConfig();
		if(empty($bepusdt['enabled'])){
			$rs['code']=1002;
			$rs['msg']=\PhalApi\T('USDT支付未开启');
			return $rs;
		}
		...
		$host=rtrim(\App\get_host(),'/');
		$name=\PhalApi\T("充值{coin}星币",array('coin'=>$coin));
		$result=\App\bepusdtCreateTransaction(
			$orderid,
			$money,
			$name,
		...
		if(($result['status_code'] ?? 0) != 200){
			$rs['code']=1005;
			$rs['msg']=$result['message'] ?? \PhalApi\T('USDT订单创建失败');
			$rs['info'][0]['orderid']=$orderid;
			return $rs;
		}
		...
		if(!empty($data['trade_id'])){
			\PhalApi\DI()->notorm->charge_user
				...
				->update(array('trade_no'=>$data['trade_id']));
		}
	 ...
	 * @return int info[0]['list'][]['coin'] 返回充值规则星币数
