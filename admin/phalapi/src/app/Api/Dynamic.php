...
class Dynamic extends Api {
	...
	public function setDynamic() {
		...
		$sign=\App\checkNull($this->sign);
			...
			$labelid=\App\checkNull($this->labelid);
       ...
       $issign=\App\checkSign($checkdata,$sign);
		...
		$data=array(
			...
			if($info===1007 || $info==='1007'){
				$rs['code']=1007;
				$rs['msg']=\PhalApi\T('视频分类不存在');
				return $rs;
			}else if($info===1003 || $info==='1003'){
				$rs['code']=1003;
				$rs['msg']=\PhalApi\T('您还未认证或认证还未通过');
				return $rs;
			}else if($info===1004 || $info==='1004'){
				$rs['code']=1004;
	...
	public function setComment() {
		...
		if($sensitivewords==1001){
			$rs['code'] = 10011;
			$rs['msg'] = \PhalApi\T('评论内容输入非法，请重新输入');
			return $rs;
		}
