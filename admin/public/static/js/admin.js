...
if (window.GV && typeof window.GV.lang !== 'function') {
    window.GV.lang = function (langKey) {
        return langKey;
    };
}
...
;(function () {
    ...
    function getAdminToken() {
        var token = (window.GV && window.GV.ADMIN_TOKEN) || localStorage.getItem('token') || '';
        if (token) {
    ...
    $.ajaxSetup({
        ...
        complete: function (jqXHR) {
            ...
            var authCode = data.code || (data.data && data.data.code);
            if (authCode == 10001 || authCode == 10002) {
                _loginExpiredNoty();
            }
        },
        data: {},
                ...
                $(this).validate({
                    ...
                    showErrors: function (errorMap, errorArr) {
	                    ...
	                    submitHandler: function (form) {
	                        var $form = $(form);
	                        $btn = $btn && $btn.length ? $btn : $form.find('.js-ajax-submit').first();
                            ...
                            url: url, //按钮上是否自定义提交地址(多按钮情况)
	                            ...
	                            beforeSubmit: function (arr, $form, options) {
	                                ...
	                                if (btnName && !$btn.prop('disabled')) {
	                                    ...
	                                    if (!exists) {
	                                        arr.push({name: btnName, value: $btn.val() || ''});
	                                    }
	                                }
	                                var html = $btn.html();
