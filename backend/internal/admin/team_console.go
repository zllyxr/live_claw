...
func (h *Handler) teamLogin(w http.ResponseWriter, r *http.Request) {
	...
	if errors.Is(err, auth.ErrInvalidCredentials) {
		httpx.Error(w, httpx.RequestID(r.Context()), http.StatusUnauthorized, 401, "用户账号或密码错误")
		return
	}
	if err != nil {
		httpx.Error(w, httpx.RequestID(r.Context()), http.StatusInternalServerError, 500, "团队登录暂不可用")
	...
	if errors.Is(err, sql.ErrNoRows) {
		httpx.Error(w, httpx.RequestID(r.Context()), http.StatusForbidden, 403, "该账号不是有效团队负责人")
		return
	}
	if err != nil {
		httpx.Error(w, httpx.RequestID(r.Context()), http.StatusInternalServerError, 500, "团队登录暂不可用")
...
func (h *Handler) createTeamSession(r *http.Request, principal teamConsolePrincipal) (teamConsoleSession, error) {
	...
	if _, err = tx.ExecContext(r.Context(), `
		INSERT INTO team_console_sessions
			(id,user_id,token_hash,csrf_hash,ip,user_agent,expires_at)
		VALUES(?,?,?,?,?,?,?)`,
		sessionID, principal.UserID, tokenHash, csrfHash, ip, userAgent, expiresAt,
	); err != nil {
		return teamConsoleSession{}, err
	}
	if _, err = tx.ExecContext(r.Context(), `
		...
		VALUES(?,2,?,'team.login','team',?,?,?,JSON_OBJECT('team_code',?,'session_id',?))`,
		httpx.RequestID(r.Context()), principal.UserID, principal.TeamID, ip, userAgent,
...
func (h *Handler) teamLogout(w http.ResponseWriter, r *http.Request) {
	...
	if _, err = tx.ExecContext(r.Context(), `
		...
		VALUES(?,2,?,'team.logout','team',?,?,?)`,
		httpx.RequestID(r.Context()), principal.UserID, principal.TeamID,
...
func (h *Handler) listOwnTeamMembers(w http.ResponseWriter, r *http.Request) {
	principal, _ := teamPrincipalFromRequest(r)
	h.listScopedTeamMembers(w, r, principal.TeamID, principal.UserID)
}
...
func (h *Handler) currentTeamPrincipal(r *http.Request) (teamConsolePrincipal, error) {
	...
	err := h.db.QueryRowContext(r.Context(), `
		...
		JOIN users user ON user.id=session.user_id AND user.status=1
		WHERE session.token_hash=? AND session.revoked_at IS NULL
		  AND session.expires_at>CURRENT_TIMESTAMP(3) LIMIT 1`,
	...
	principal, err := h.loadTeamPrincipal(r.Context(), userID)
	if err != nil {
		return teamConsolePrincipal{}, err
	}
...
func (h *Handler) loadTeamPrincipal(ctx context.Context, userID int64) (teamConsolePrincipal, error) {
	...
	err := h.db.QueryRowContext(ctx, `
		...
		&principal.UserID, &principal.Nickname, &principal.TeamID,
...
func teamPrincipalForAPI(principal teamConsolePrincipal) map[string]any {
	return map[string]any{
		"user_id": apiDecimalID(principal.UserID), "nickname": principal.Nickname,
		"team": map[string]any{
