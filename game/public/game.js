...
const launchUID = Number(launchQuery.get("uid"));
const launchName = normalizeName(launchQuery.get("name"));
const hasLaunchTicket = Number.isInteger(launchTable) && launchTable >= 1 && launchTable <= 1000
  && Number.isSafeInteger(launchUID) && launchUID > 0
  && launchQuery.get("ts") && launchQuery.get("sig");

if (hasLaunchTicket) {
  const matchedRoom = `T${String(launchTable).padStart(4, "0")}`;
  state.profile = {
    name: launchName || `玩家${launchUID}`,
    roomId: matchedRoom,
    uid: launchUID,
    ts: launchQuery.get("ts"),
    sig: launchQuery.get("sig"),
    table: launchTable
  };
