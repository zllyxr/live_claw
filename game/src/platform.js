...
export function verifyLaunchTicket(input, secret, now = Date.now()) {
  ...
  const expected = createHmac('sha256', secret)
    ...
    .digest('hex')
    .slice(0, 32);
  if (!secret || !safeEqual(expected, input?.sig)) throw new Error('进入凭证校验失败');
  return {
    uid,
    table,
    roomId: `T${String(table).padStart(4, '0')}`,
...
export class PlatformWallet {
  ...
  async request(path, payload) {
    ...
    const response = await this.fetch(`${this.baseURL}${path}`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', 'X-Minigame-Secret': this.secret },
