...
: "${DB_HOST:?Set DB_HOST, for example live.travelig.vip}"
: "${DB_USER:?Set DB_USER}"
...
MYSQL=(mysql --connect-timeout=10 --default-character-set="$MYSQL_CHARSET" -h"$DB_HOST" -P"$DB_PORT" -u"$DB_USER" -p"$DB_PASSWORD")
MYSQLDUMP=(mysqldump --default-character-set="$MYSQL_CHARSET" -h"$DB_HOST" -P"$DB_PORT" -u"$DB_USER" -p"$DB_PASSWORD" --single-transaction --routines --triggers)

mkdir -p "$BACKUP_DIR"

if "${MYSQL[@]}" -N -e "SHOW DATABASES LIKE '$DB_NAME'" | grep -qx "$DB_NAME"; then
  BACKUP_FILE="$BACKUP_DIR/${DB_NAME}_${DB_HOST}_before_utf8_reimport_${TIMESTAMP}.sql"
  echo "Backing up $DB_NAME@$DB_HOST to $BACKUP_FILE"
  "${MYSQLDUMP[@]}" "$DB_NAME" > "$BACKUP_FILE"
fi

echo "Recreating database $DB_NAME on $DB_HOST with $MYSQL_CHARSET"
